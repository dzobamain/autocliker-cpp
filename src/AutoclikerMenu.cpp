...
void AutoclikerMenu::display_settings()
{
    ...
    std::string settings_text = lang.at("SETTINGS_TITLE") + "\n" +
                                (lang.at("SETTINGS_NUMBER").find("%d") != std::string::npos ? (lang.at("SETTINGS_NUMBER").substr(0, lang.at("SETTINGS_NUMBER").find("%d")) + std::to_string(set.number_of_clicks)) : lang.at("SETTINGS_NUMBER")) + "\n" +
...
void AutoclikerMenu::change_language()
{
    ...
    if (lang_item >= 0 && lang_item < static_cast<int>(language_codes.size()))
    {
        set.language = language_codes[lang_item];
